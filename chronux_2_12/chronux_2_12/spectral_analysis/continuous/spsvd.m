function [sv,sp,fm,tapers,tvec,f,data,u,s,v,a,proj] = spsvd(data,params,mdkp)
...
nf=length(f);
if exist('anaType','var')
    % Modified from original Chronux by Sebastien Proulx 2023
    switch anaType
        case 'proulx'
            ...
            a=data'*proj(:,:); % projected data (chan X taper+freq)
            [u,s,v]= svd(a,0); % svd

            %     sv = diag(sv(1:mdkp,1:mdkp))';
            sv = diag(s)';
            sp = conj(permute(u(:,1:mdkp),[1 3 2]));
            fm = conj(permute( reshape( permute(v(:,1:mdkp),[2 1]) ,[mdkp K nf]) ,[2 3 1]));
    ...
    if nf>10
        % Original Mitra, but with parfor
        parfor j=1:nf
            %     for k=1:K
