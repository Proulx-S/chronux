function [S,Sc,f,Serr]=mtspectrumc2(data,params)
...
% %%%%%%
% dataX = data;
% dataPh = angle(data); dataRho = abs(data);
% dataRho = dataRho + mean(dataRho);
% [X,Y] = pol2cart(dataPh,dataRho);
% dataX = complex(X,Y);
% Jx=mtfftc(dataX,tapers,nfft,Fs);
% Jx = fftshift(Jx);
% Jx=Jx(findx,:,:);
% % hPlot = plot(f,abs(Jx),'linewidth',2); hold on
% hPlot = plot(f,real(Jx)); hold on
% hPlot2 = plot(f,imag(Jx),':'); hold on
% for i = 1:length(hPlot)
%     hPlot2(i).Color = hPlot(i).Color;
% end
% yyaxis right
% hPlot3 = plot(f,angle(Jx));
% for i = 1:length(hPlot)
%     hPlot3(i).Color = hPlot(i).Color;
% end
% %%%%%%
